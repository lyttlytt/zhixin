...
//    }
     ...
     * @param path
     */
    public void insertBitmap(Uri path) {
        Bitmap bitmap = getSmallBitmap1(path, 480, 800);
        insertBitmap(path.toString(), bitmap);
    }
    public void insertBitmap(String url,Uri localPath){
        Bitmap bitmap = getSmallBitmap1(localPath,480,800);
    ...
    // 根据路径获得图片并压缩，返回bitmap用于显示
    public Bitmap getSmallBitmap1(Uri filePath, int reqWidth, int reqHeight) {
//        final BitmapFactory.Options options = new BitmapFactory.Options();
//        options.inJustDecodeBounds = true;
//        BitmapFactory.decodeFile(filePath, options);
//
...
//        int w_height = w_width * b_height / b_width;
//        bitmap = Bitmap.createScaledBitmap(bitmap, w_width, w_height, false);
        Bitmap bitmap = BitmapUtils.decodeUri(getContext(), filePath, 400, 800);
        return bitmap;
    }
    //计算图片的缩放值
